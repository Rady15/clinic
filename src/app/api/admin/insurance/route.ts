...
export async function GET(_request: NextRequest) {
  ...
  try {
    const items = await db.insuranceCompany.findMany({ orderBy: { order: 'asc' } });
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const item = await db.insuranceCompany.create({ data });
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const item = await db.insuranceCompany.update({ where: { id }, data: updateData });

    ...
    return (
      <main className="min-h-screen flex items-center justify-center bg-gray-50">
        ...
        >
          ...
          <h2 className="text-2xl font-bold text-[#2C3E50] mb-3">سلة مشترياتك فارغة حالياً</h2>
          <p className="text-[#7F8C8D] mb-6">قم بإضافة خدمات قبل المتابعة للدفع</p>
          <Button
            onClick={() => setCurrentPage('services')}
            className="bg-[#6DB3D7] hover:bg-[#5DADE2] text-white px-8 rounded-xl"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            {items.map(item => (
              ...
              >
                <div className="w-20 h-20 bg-[#EBF5FB] rounded-xl flex items-center justify-center shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    {item.originalPrice && (
                      <span className="text-xs text-[#7F8C8D] line-through">{item.originalPrice.toLocaleString()} ر.س</span>
                    )}
                    <span className="font-bold text-[#6DB3D7]">{item.price.toLocaleString()} ر.س</span>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm h-fit sticky top-28">
            <h3 className="font-bold text-[#2C3E50] text-lg mb-6">ملخص الطلب</h3>
            <div className="space-y-3 mb-6">
              <div className="flex justify-between text-sm">
                <span className="text-[#7F8C8D]">المجموع الفرعي</span>
                <span className="font-medium">{total.toLocaleString()} ر.س</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-[#7F8C8D]">الضريبة (15%)</span>
                <span className="font-medium">{(total * 0.15).toFixed(0)} ر.س</span>
              </div>
              <div className="border-t border-gray-100 pt-3 flex justify-between">
                <span className="font-bold text-[#2C3E50]">الإجمالي</span>
                <span className="font-bold text-[#6DB3D7] text-xl">{(total * 1.15).toFixed(0)} ر.س</span>
              </div>
            </div>
            ...
            <button
              onClick={() => setCurrentPage('services')}
              className="w-full text-center text-sm text-[#6DB3D7] font-medium mt-3 hover:underline"
            >
              متابعة التسوق
            </button>
          </div>

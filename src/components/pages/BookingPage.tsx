...
const steps = [
  ...
  { labelAr: 'البيانات والتأكيد', labelEn: 'Details & Confirm', icon: CreditCard },
];
...
export default function BookingPage() {
  ...
  return (
    <main>
      <HeroSection titleKey="booking.title" pageKey="booking" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[500px] overflow-y-auto">
                {doctors.map(doctor => (
                  <button
                    key={doctor.id}
                    onClick={() => {
                      setSelectedDoctor(locale === 'en' ? doctor.nameEn : doctor.nameAr);
                      setSelectedDoctorId(doctor.id);
                      setSelectedDept(locale === 'en' ? doctor.departmentEn : doctor.departmentAr);
                      setStep(2);
                    }}
                    className="p-4 rounded-xl text-right transition-all flex items-center gap-3 bg-white/20 text-white hover:bg-white/30 hover:shadow-md"
                  >
                    {doctor.image ? (
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-white/60 mt-0.5">
                        {locale === 'en' ? doctor.departmentEn : doctor.departmentAr}
                      </p>
                    </div>
                  </button>
                ))}
                {doctors.length === 0 && (
                  <p className="text-white/80 col-span-2 text-center py-8">{locale === 'en' ? 'No doctors available' : 'لا يوجد أطباء متاحين'}</p>
                )}
              </div>
            </motion.div>
          )}
          ...
          {step === 2 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <div className="bg-white rounded-xl p-6 space-y-4">
                <div className="bg-[#EBF5FB] rounded-xl p-4 text-sm mb-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input placeholder={locale === 'en' ? 'Enter your phone' : 'أدخل رقم جوالك'} value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} className="h-11 rounded-xl" dir="ltr" required />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-[#333] mb-1">{t('booking.email', locale)}</label>
                  <Input type="email" placeholder={locale === 'en' ? 'Enter your email' : 'أدخل بريدك'} value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className="h-11 rounded-xl" dir="ltr" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-[#333] mb-1">{t('booking.date', locale)}</label>
                    <Input type="date" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} className="h-11 rounded-xl" dir="ltr" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[#333] mb-1">{t('booking.time', locale)}</label>
                    <Input type="time" value={form.time} onChange={e => setForm({ ...form, time: e.target.value })} className="h-11 rounded-xl" dir="ltr" />
                  </div>
                </div>
                <div>
              ...
              <div className="mt-6 flex justify-between">
                <Button onClick={() => { setStep(1); setSelectedDoctor(''); setSelectedDoctorId(''); setSelectedDept(''); }} variant="ghost" className="text-white hover:bg-white/20 rounded-xl">
                  {locale === 'en' ? 'Change Doctor' : 'تغيير الطبيب'}
                </Button>
